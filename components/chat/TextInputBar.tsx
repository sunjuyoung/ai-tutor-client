 ...
 * - Phase 3.5: 마이크 버튼으로 음성 녹음, 음성 모드 토글
 */
...
interface TextInputBarProps {
  ...
  /** Phase 3.5: 녹음 완료 시 오디오 Blob 전달 */
  onAudioSend?: (blob: Blob) => void;
  disabled?: boolean;
  /** Phase 3.5: 녹음 중 여부 (외부에서 제어) */
  isRecording?: boolean;
  /** Phase 3.5: 녹음 시작 핸들러 */
...
const TextInputBar = forwardRef<TextInputBarRef, TextInputBarProps>(
  function TextInputBar({ onSend, disabled, isRecording, onStartRecording, onStopRecording }, ref) {
    ...
    // Phase 3.5: 마이크 버튼 클릭 — 녹음 시작/중지 토글
    ...
    return (
      <div className="flex items-end gap-2 px-4 py-3 bg-white border-t border-gray-100">
        ...
        <button
          onClick={handleSend}
          disabled={disabled || !hasText || isRecording}
